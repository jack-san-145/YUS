...
func (pg *PgStore) AssignDriverToBus(ctx context.Context, driverID int, busID int) error {
	...
	if is_driver_present {
		...
		_, err = pool.Exec(ctx, query, driverID, busID)
		if err != nil {
...
func (pg *PgStore) AssignRouteToBus(ctx context.Context, routeID int, busID int) error {
	...
	if is_route_present && routeID != 0 {
		...
		_, err = pool.Exec(ctx, query, routeID, route_name, src, dest, busID)
		if err != nil {
...
func (pg *PgStore) GetSrcDestNameByRouteID(ctx context.Context, routeID int) (string, string, string, error) {
	...
	err := pool.QueryRow(ctx, query, routeID).Scan(&route_name, &src, &dest)
	if err != nil {
...
func (pg *PgStore) RouteExistsInCBR(ctx context.Context, routeID int) (bool, error) {
	...
	err := pool.QueryRow(ctx, query, routeID).Scan(&is_route_present)
	if err != nil {
...
func (pg *PgStore) DriverExistsInCBR(ctx context.Context, driverID int) (bool, error) {
	...
	err := pool.QueryRow(ctx, query, driverID).Scan(&is_driver_present)
	if err != nil {
...
func (pg *PgStore) UpdateBusRoute(ctx context.Context, route *models.BusRoute) error {
	...
	_, err := pool.Exec(ctx, query, route.RouteId)
	if err != nil {
	...
	_, err = pool.Exec(ctx, query, route.RouteId, route.RouteName, route.Src, route.Dest, route.BusID)
	if err != nil {
		fmt.Println("error while update the route_id for given bus_id - ", err)
		return err
	}
...
func (pg *PgStore) UpdateBusDriver(ctx context.Context, driverID int, busID int) error {
	...
	_, err := pool.Exec(ctx, query, driverID)
	if err != nil {
...
func (pg *PgStore) AddBus(ctx context.Context, busID int) error {
	...
	err := pool.QueryRow(ctx, query, busID).Scan(&is_bus_exists)
	if err != nil {
	...
	_, err = pool.Exec(ctx, query, busID)
	if err != nil {
...
func (pg *PgStore) CacheRoute(ctx context.Context, route *models.BusRoute) error {
	...
	err := pool.QueryRow(ctx, query, route.BusID, route.RouteId).Scan(&is_route_exists)
	if err != nil {
	...
	if !is_route_exists {
		...
		_, err = pool.Exec(ctx, query, route.BusID, route.RouteId, route.RouteName, route.Src, route.Dest)
		if err != nil {
