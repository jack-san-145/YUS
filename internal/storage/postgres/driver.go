...
func (pg *PgStore) GetAllottedBusForDriver(ctx context.Context, driverID int) (models.AllotedBus, error) {
	...
	if errors.Is(err, sql.ErrNoRows) {
		...
		log.Println("error while finding the allotted bus for driver - ", err)
...
func (pg *PgStore) DriverExists(ctx context.Context, driverID int) (bool, error) {
	...
	if err != nil {
		log.Println("error while checking the existance of driver - ", err)
		return exists, err
	}
	return exists, nil
}
...
func (pg *PgStore) SetDriverPassword(ctx context.Context, driverID int, email string, password string) (bool, error) {
	...
	if exists, _ := pg.DriverExists(ctx, driverID); exists {
		...
		if err != nil {
			log.Println("error while update the driver's password - ", err)
			return false, err
		}
	}
	return true, nil
...
func (pg *PgStore) ValidateDriver(ctx context.Context, driverID int, password string) (bool, error) {
	...
	if err != nil {
		log.Println("error while validate the driver - ", err)
		return false, err
	}
...
func (pg *PgStore) GetAvailableDrivers(ctx context.Context) ([]models.AvailableDriver, error) {
	...
	if err != nil {
		log.Println("error while selecting the driver_id , driver_name and mobile no - ", err)
		return nil, err
	}
	...
	for all_drivers.Next() {
		...
		if err != nil {
			log.Println("error while scanning the driver from DB - ", err)
			continue
		}
		...
		if err != nil {
			log.Println("error while checking existance of the driver in current_bus_route - ", err)
			continue
		}
		if !is_driver_exists {
